...
const runModes = [
  ...
  { key: 'full', label: 'Full Karnataka Run', budget: 4000, note: 'Deep Karnataka discovery using the corrected seed-pattern logic.' },
  { key: 'extended', label: 'Extended Run', budget: 5500, note: 'Maximum Karnataka discovery. Seed-pattern and niche pathways get priority.' },
...
const pathwayOptions = [
  ...
  { key: 'full_day_alternative', label: 'Whole-child / alternative education', note: 'Christel House / Parikrma / Kaliyuva Mane-type schools, dropouts, K-12, college/career pathways.' },
...
function HowItWorks({ open, onToggle }: { open: boolean; onToggle: () => void }) {
  return <div className="form-card" style={{marginTop: 0}}>
    ...
    {open && <div className="advanced-panel" style={{margin:'14px 0 0'}}>
      <p className="muted-empty">It searches for organisations that look like Christel House, Parikrma, Mahesh Foundation, SVYM, Bridges of Sports, KSV, Kaliyuva Mane and similar child-pathway institutions. It removes wrong-state results, donation-page noise, generic special schools, generic CCIs, weekend volunteering and weak tuition-type rows. The final list is not scored; it is meant for human review.</p>
    </div>}
  </div>
}
  ...
  return <><Header active="repository"/><main className="dfp-wrap page-stack">
    ...
    <section className="discover-card">
      ...
      {tab==='general'&&<>
        ...
        <div className="budget-strip"><b>{Number(budget||0).toLocaleString()} / {MAX_DISCOVERY_BUDGET.toLocaleString()} query cap</b><span>{runModes.find(m=>m.key===runMode)?.note || 'Manual query budget'} · The run does not score NGOs; it uses seed-pattern logic and surfaces a cleaner list for manual sorting.</span></div>
